...
This is equivalent to the model given on page 101 of the [Reference Manual](http://mc-stan.org/users/documentation/). We use the SlicStan compiler to translate the program to Stan, which results in the same Stan model:
...
As the program is translated to Stan, it could readily be used with already available useful Stan tools, such as [bayesplot](http://mc-stan.org/users/interfaces/bayesplot.html) and [ShinyStan](http://mc-stan.org/users/interfaces/shinystan.html). For example, we can use RStan to compile the model above, and bayesplot to plot results or diagnostics:
...
The rest of this section briefly discusses the idea behind information flow analysis and outlines the way each of the translation steps is implemented. A more elaborate explanation, including formal syntax and typing rules of SlicStan, is presented in the [SlicStan MSc thesis](http://homepages.inf.ed.ac.uk/s1207807/files/slicstan.pdf).
...
* <span style="font-variant:small-caps;">low</span> --- such variables hold *low* security, public data.
* <span style="font-variant:small-caps;">high</span> --- such variables hold *high* security, secret data.


We want to disallow the flow of secret information to a public variable, but allow other flows of information. That is, if `L` is a variable of security level <span style="font-variant:small-caps;">low</span>, and `H` is a variable (or an expression) of security level <span style="font-variant:small-caps;">high</span>, we want to forbid statements such as `L = H`, but allow:

* `L = L`
* `H = H`
* `H = L`

In other words, if we define an order $<$ on the levels, so that <span style="font-variant:small-caps;">low</span> $<$ <span style="font-variant:small-caps;">high</span>, secure information flow analysis is used to ensure that information flows only upwards with respect to that ordering. This is also known as the *noninterference property* --- changes to confidential inputs lead to no changes in public outputs of a system [@GoguenMeseguer]. 
...
2. If `x` is declared *without* the keyword `data`, and `x` is *not assigned to* anywhere in the program, then <span style="font-variant:small-caps;">model</span> $\leq$`level(x)`. In other words, if we never assign to a non-data variable, then this variable is either a parameter or an unused generated quantity.
3. If `x = foo(y)` for any function `foo`, then `level(y)` $\leq$ `level(x)`.
...
For the simple example above, this means that we will chose `variance` to be of level <span style="font-variant:small-caps;">genquant</span> rather than level <span style="font-variant:small-caps;">model</span>, as <span style="font-variant:small-caps;">genquant</span> $<$ <span style="font-variant:small-caps;">model</span>.
...
* all sampling statements belong to the `model` block.
...
The need to statically unroll calls to user-defined functions is one of the biggest current limitations of SlicStan. It is not possible to compile a program that contains loops with run-time bounds, and Stan's output can become less readable, due to the lack of naming control for local variables. 
...
Another simple example showing SlicStan ability to deduce the Stan roles of variables automatically, is one adapted from a [blogpost by Bob Carpenter](http://discourse.mc-stan.org/t/generalizing-the-stan-language-for-stan-3/1599), where he discusses the possibility of Stan changing in a similar blockless direction:
...
This model is equivalent to the non-centred Stan model given in the [Reference Manual](http://mc-stan.org/users/documentation/):
```{Stan}
parameters {
    real y_raw;
    real x_raw;
...
SlicStan successfully applies static analysis to allow Stan programs to be more concise, and easier to write and modify. This opens a range of possibilities for future work that uses programming language techniques to aid probabilistic programming and inference. For example, a future version of SlicStan could support vectorisation of user-defined functions, which would allow for models to be specified even more compactly. Constraints on the range that parameters can have (`real <lower=0> sigma` for instance) could be automatically deduced using type inference. Semantic-preserving transformations could be applied, depending on the specific program, to compile to a better optimised and better behaved sampling algorithm. 
